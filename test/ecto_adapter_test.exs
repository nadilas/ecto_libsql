defmodule Ecto.Adapters.LibSqlExTest do
  use ExUnit.Case, async: false

  alias Ecto.Adapters.LibSqlEx
  ...
  describe "remote-only mode" do
    ...
    test "storage_down returns error for remote-only mode" do
      opts = [uri: "libsql://example.turso.io", auth_token: "token"]
      assert {:error, :not_supported} == LibSqlEx.storage_down(opts)
    end
  ...
  describe "type loaders" do
    ...
    test "loads datetime from string" do
      loader = LibSqlEx.loaders(:naive_datetime, :naive_datetime) |> List.first()
    ...
    test "loads date from string" do
      loader = LibSqlEx.loaders(:date, :date) |> List.first()
    ...
    test "loads time from string" do
      loader = LibSqlEx.loaders(:time, :time) |> List.first()
  ...
  describe "type dumpers" do
    test "dumps boolean to integer" do
      dumper = LibSqlEx.dumpers(:boolean, :boolean) |> List.last()
      assert {:ok, 0} == dumper.(false)
      assert {:ok, 1} == dumper.(true)
    end

    test "dumps datetime to string" do
      dumper = LibSqlEx.dumpers(:naive_datetime, :naive_datetime) |> List.last()
    ...
    test "dumps date to string" do
      dumper = LibSqlEx.dumpers(:date, :date) |> List.last()
      date = ~D[2024-01-15]
    ...
    test "dumps time to string" do
      dumper = LibSqlEx.dumpers(:time, :time) |> List.last()
      time = ~T[10:30:00]
    ...
    test "dumps binary with blob wrapper" do
      dumper = LibSqlEx.dumpers(:binary, :binary) |> List.last()
